function [dsdt] = TC2_coco(s,par)
    % Low order tropical cyclone model, syntax to use with COCO continuation
    ...
    T_s = par(1,:); % (K) Sea surface temperature 
    h_a = par(2,:); %0.45; % (%) Relative humidity, ambient region
    p_a = 500;
    h_refb = par(3,:); %0.8; % (%) Relative humidity, boundary layer
    p_ref = 1000;
    beta = par(4,:); % Tangential wind profile parameter
    ...
    dsdt(1,:) = 3600.*(stream_b2(s(1,:),par).*(s(2,:)-s(1,:))./M_i) + (s_as-s(1,:))./tau_E;
    
    % change in eyewall boundary layer entropy
    dsdt(2,:) = 3600.*(stream_b2(s(1,:),par).*(s(3,:)-s(2,:))./M_bi(s(1,:),par) + (C_H./(2.*H_b)).*(abs(v_b1(s(1,:),par))+abs(v_b2(s(1,:),par))).*(s_oi(s(1,:),par)-s(2,:)));
    
    % change in ambient region boundary layer entropy
    dsdt(3,:) = 3600.*(stream_b2(s(1,:),par).*(del.*s_a-s(3,:))./M_ba(s(1,:),par) + (C_H./(2.*H_b)).*abs(v_b2(s(1,:),par)).*(s_oa(s(1,:),par)-s(3,:))) + (s_a-s(3,:))./tau_C; 
